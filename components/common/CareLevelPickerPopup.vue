<template>
  <view v-if="shown" class="mask" @tap="emit('close')">
    <view class="sheet" @tap.stop>
      <text class="sheet__title ff-yuan">护理等级</text>
      ...
      <view class="sheet__picker-wrap">
        ...
        >
          <picker-view-column>
            <view v-for="(item, i) in options" :key="'care-' + i" class="sheet__pv-row">
...
function syncIndex() {
  ...
  const idx = Number(props.initialIndex)
  pickIdx.value =
    Number.isFinite(idx) && idx >= 0 && idx < list.length ? idx : 0
}
...
watch([() => props.initialIndex, () => props.options], () => {
